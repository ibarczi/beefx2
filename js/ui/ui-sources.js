...
import {Corelib, DOMplusUltra} from '../improxy.js'
...
export const extendUi = async ui => { //: input: ui.sourceStrip$ (empty)
  ...
  //#595  Youtube/p3 menu for testing on dev site
  ...
  window.fetch('/au/add-audio-esm.js') //: private files, if not on pub site (->beeFx.getFullPath!)
    .then(resp => resp.ok
      ? import(resp.url)
          ...
          .catch(err => pinky.mp3s.resolve([])) // eslint-disable-line
      : pinky.mp3s.resolve([])
    )
    .catch(err => pinky.mp3s.resolve([])) // eslint-disable-line
  ...
  //#c95  Youtube mock stuff - if we are not on Youtube, so we have to replace videos with audio
  ...
  ui.changeAudioSource = (sourceIx, {src, title, backgroundImage}) => { //#2b2  [audio]
  ...
  ui.changeVideoElementSource = (sourceIx, video$) => { //#2b2  [master]
    const sourceUi = prepareSourceChange(sourceIx)
  ...
  ui.changeVideoSource = (sourceIx, {videoId, title, src}) => { //#2b2  [mock / video]
    const sourceUi = prepareSourceChange(sourceIx)
    ...
    if (on) {
      ...
      if (root.onYoutube) {
        for (const thumb of q$$('a#thumbnail')) {
          ...
          thumbs.push({thumb, videoId, attach: thumb.parentElement})
        }
      }
      for (const thumb of q$$('div#thumbnail.emulated')) {
        ...
        thumbs.push({thumb, attach: thumb, src, videoId, title})
      }
      for (const {attach, videoId, src = '', title = ''} of thumbs) {
        if (videoId?.length === 11 || src) {
          div$(attach, {class: 'bfx-grab-frame', attr: {videoId, title, src}},
            '1234'.split('').map(text =>
