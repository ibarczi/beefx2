/* eslint-disable no-debugger, spaced-comment, no-multi-spaces, valid-typeof, 
   ...
   no-void, quotes, no-floating-decimal, import/first, space-unary-ops, no-empty,
   object-curly-newline */
   
import * as Im from '../improxy.js'
...
export const createUI = (root, exroot) => {
  ...
  ui.installKeyboardListener = _ => document.addEventListener('keyup', event => {
    ...
    if (key === 'F2') { //FUTREQ: fsd df sfsdf s ds fsd fsfsd
      ui.setFlag('fxfactory')
    } else if (key === 'F4') { //TODO: ksksksks
      ui.setFlag('sourceList') 
    } else if (key === 'F7') {
    } else if (key === 'F9') {
      ui.setFlag('sourceList') //DEBUG dfdfg
      if (ctrlKey) {
      } else if (shiftKey) { //OPTIMIZE trtrtr
      } else {
      }
    } else if (key === 'Shift') {
    }
