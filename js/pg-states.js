...
import {Corelib, DOMplusUltra, Store} from './improxy.js'
...
const stagePresets = { //: These compressed defs are ugly, but it's easier to overview.
  ...
  youtubeTint: {
    ABCD: 'b,bi,bi,b,b,spect,scope'},
  youtubeTint2: {
    A: 'bi,bi,b,spect,scope',
    B: 'b,customBiquad,customBiquad,spect,scope'},
...
//: name->ret, parent$->menu
  stateManager.getActualPreset = async ({name, parent$}) => new Promise(resolve => {
    ...
    if (name) {
      if (name === 'last') {
        const startup = store.load('onStartup') || {type: 'actPreset', presetName: 'preset4xb'}
        if (startup.type === 'actPreset') {
          const lastStored = startup.presetName
          ui.setFlag('fxfactory', true)
  ...
  stateManager.getProjectList = _ => {
    ...
    store.filterKeysIntoArray('project#').map(projName => ret.push(projName.split('project#')[1]))
    /*
    const dbg = []
